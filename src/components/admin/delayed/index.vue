          ...
          <table class="table table-hover m-b-0">
            ...
            <tbody>
              <tr v-for="(order,i) in categories" :key="i">
                ...
                <td>
                  <span><a :href="'/admin/product/edit/'+order.product._id">{{order.product.title.ar | truncate(30, '...')}}</a></span>
                </td>
                <td>
  ...
  methods: {
    async getCategories() {
      const res = await axios({
        url: `/admin/delayed-order/paginate?page=${this.page}&limit=${this.limit}`
      });
    ...
    makeAvailable(cat, i) {
      axios({
        url: "/admin/delayed-order/update",
        method: "PUT",
        data: {
          id: cat._id
        }
